...
# -------- TOOLS FOR CREATE GRAPHICS  ---------------
    ...
    def get_tool_bar_chart(self):
        @tool
        def tool_bar_chart(column_name1: str, column_name2: str, color: str = 'red') -> str:
            """
            Create a bar chart with column_name1 (categorical) on the x-axis and column_name2 (numerical) as the height of the bars.
            """
            # Validate that column_name1 is categorical and column_name2 is numerical
            if not pd.api.types.is_categorical_dtype(self.data[column_name1]) and not pd.api.types.is_object_dtype(self.data[column_name1]):
                return f"Error: Column '{column_name1}' must be categorical for a bar chart."
            
            if not pd.api.types.is_numeric_dtype(self.data[column_name2]):
                return f"Error: Column '{column_name2}' must be numerical for a bar chart."
            
            fig, ax = plt.subplots()
            # Create the bar chart
            self.data.plot(kind='bar', x=column_name1, y=column_name2, color=color, ax=ax, legend=False)
            ax.set_title(f"Bar Chart: {column_name1} vs {column_name2}")
            ...
            chart_name = f"chart_{column_name1}_{column_name2}.png"
            fig_path = os.path.join('./graphs', chart_name)
    ...
    def get_tool_line_chart(self):
        @tool
        def tool_line_chart(column_name1: str, column_name2: str, color1: str = 'red', color2: str = 'blue') -> str:
            """
            Create a line chart with column_name1 on the x-axis and column_name2 on the y-axis.
            """
            fig, ax = plt.subplots()
            ax.plot(self.data[column_name1], self.data[column_name2], color=color1)
            ax.set_title(f"Line Chart: {column_name1} vs {column_name2}")
            ax.set_xlabel(column_name1)
            ax.set_ylabel(column_name2)
            ...
            chart_name = f"line_chart_{column_name1}_{column_name2}.png"
            fig_path = os.path.join('./graphs', chart_name)
            fig.savefig(fig_path)
    ...
    def get_tool_scatter_plot(self):
        @tool
        def tool_scatter_plot(column_name1: str, column_name2: str, color1: str = 'red', color2: str = 'blue') -> str:
            """
            Create a scatter plot with column_name1 on the x-axis and column_name2 on the y-axis.
            """
            fig, ax = plt.subplots()
            ax.scatter(self.data[column_name1], self.data[column_name2], color=color1)
            ax.set_title(f"Scatter Plot: {column_name1} vs {column_name2}")
            ax.set_xlabel(column_name1)
            ax.set_ylabel(column_name2)
            ...
            chart_name = f"scatter_plot_{column_name1}_{column_name2}.png"
            fig_path = os.path.join('./graphs', chart_name)
            fig.savefig(fig_path)
